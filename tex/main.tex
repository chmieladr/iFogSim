...
\author{Szymon Bedeniczuk, Adrian Chmiel} %DOPISZCIE SIE 
\date{\today}
\maketitle
...
Fog Computing, oraz Edge computing są pojęciem stworzonym przez Cisco, i definiowane jest jako system obliczeń rozproszonych, który rozwija pojęcie Cloud computingu, aby pozwolić na rozszerzenie chmury obliczeniowych do urządzeń brzegowych, takich jak routery, bramy IoT, czy lokalne serwery. Komponenty aplikacji mogą działać w sposób rozproszony, korzystając z usług świadczonych przez infrastrukturę rozproszoną, gdzie zasady zarządzania zasobami kontrolują i zarządzają usługami oferowanymi przez każde urządzenie. 
Rozwiązanie to jest zapewnia większą gęstość zasobów, co pozwala również na mniejsze opóźnienia w aplikacjach mobilnych, które często tego wymagają. Stwarza to większą skalowalność rozwiązania , z uwagi na mniejsze zapotrzebiowanie na same serwery chmurowe, gdyż pozwala ono na rozwinięcie sieci przez dodanie samych urządzeń brzegowych, oraz zmniejsza to przepływ danych do chmury. Wykorzystanie Fog Computingu pomaga także w działaniu aplikjacji krytycznych, wymagających przetwarzaniu danych w czasie rzeczywistym.

\section{Cel i Zastosowanie}
iFogSim, to narzędzie stworzone do modelowania i symulacji środowisk Fog, Edge computing, oraz Internet of Things (IoT), zbudowane na fundamentach frameworku CloudSim, narzędzia do symulowania środowisk Cloud. Narzędzie docelowo umożliwia testowanie i analizowanie strategii zarządzania symulowanymi zasobami, przed stworzeniem rzeczywistej infrastruktury.
...
Kolejnym przykładem wykorzystania tego narzędzia jest infrastruktura medyczna mająca na celu monitorowanie pacjentów w czasie rzeczywistym. To zastosowanie jest przykładem sytuacji, w której niska latencja jest najważniejsza, gdyż zależeć od niej może życie lub zdrowie pacjenta~\cite{HealthCare}.

Ostatnim przykładem będzie monitoring wideo w smart city, w takowym zastosowaniu najważniejszym jest obserwacja obrazu audio i wideo, analiza go w celu wykrywania zagrożeń, a więc szybkie przetwarzanie danych w czasie rzeczywistym. Badacze porównali rozwiązanie bazujące na systemie Cloud i Fog, z czego Fog okazał się bardziej atrakcyjny~\cite{SmartCity}.

Jak widać na podanych przykładach, systemy fog są testowane przy użyciu iFogSim głównie na systemach, które aktualnie wykorzystują systemy chmurowe, lub na potencjalnych systemach, które łączą wiele ściśle powiązanych zastosowań, w celu optymalizacji, oraz koordynacji ich działań.

\section{Architektura Narzędzia}
iFogSim składa się z warstw, które współpracując ze sobą, odpowiadają za określone zadania i poprawne działanie systemu. Odzwierciedlają one architekturę IoT, w której urządzenia generują dane, które potem przekazywane do urządzeń w wyższych warstwach po analizie przekazują zadania do urządzeń wykonawczych~\cite{iFogSimInf}. 
...
Urządzenia IoT stanowią najniższą warstwę architektury, gdyż są one zestawem urządzeń odpowiedzialnych za kontakt z użytkownikiem. Przykładami urządzeń z tej warsty są między innymi urządzenia zbierające dane, takie jak sensory, kamery, mikrofony itd. Aktuatory też są urządzeniami należącymi do tej kategorii, odpowiadają one za wywoływanie sygnałów, zarządzanie oświetleniem, głośnikami, zamkami, alarmami itd. Ich działanie jest zależne od urządzeń w wyższych warstwach.

\subsection{Warstwa urządzeń Fog}
Kolejną warstwą w środowisku są urządzenia fog, które zajmują się przetwarzaniem otrzymanych danych, oraz wysyłanie sygnałów do urządzeń warstwy niższej. Do tych urządzeń należą bramy sieciowe, routery, oraz lokalne serwery. Urządzenia z tej warstwy też podlegają wewnętrznej hierarchii. Urządzenia najbliższe użytkownikowi są blisko uzytkownika, routery i bramy sieciowe. Wyżej w hierarchii są lokalne serwery, urządzenia pomiędzy chmurą a użytkownikiem, a najwyżej sama chmura.
...
Warstwa danych jest szeroko pojętym strumieniem danych przesyłanymi poprzednich dwóch warstw do szerszego systemu i z powrotem, oraz pomiędzy urządzeniami wyższych wastw, które odpowiadają za wastwę aplikacyjną. Takimi danymi mogą być odczyty z sensorów, dane z aplikacji urządzeń fog, jak i informacje przesyłane do urządzeń IoT, aktualizacje działania urządzeń i tym podobnych.

\subsection{Warstwa monitorowania infrastruktury}
Warstwa ta, jak nazwa wskazuje, odpowiedzialna jest za zbieranie i monitorowanie danych obciążenia infrastruktury. Do tych danych zalicza się wykorzystanie zasobów podzespołów, przepustowość sieci, zużycie energii, oraz informacje o stanie urządzeń IoT, oraz ich aktywności. Dane te, po zebraniu przekazywane są do warstwy zarządzania zasobami, albo być przekazane innym servisom w ramach potrzeby.
\subsection{Warstwa zarządzania zasobami}
Zarządzanie zasobami jest głównym komponentem infrastruktury. Składa się ona z komponentów zarządzająch zasobami warstwy urządzeń fog, w celu osiągnięcia celów warstw aplikacji, przy minimalnym wykorzystaniu zasobów. 
Warstwa korzystając z informacji na temat zasobów zebrane z warstwy poprzedniej identyfikuje najlepsze możliwe urządzenia do hostowania aplikacji, oraz alokowania zasobów na nich. Może ona także wprowadzać zasady dla infrastruktury, w celu jak najlepszego wykorzystania zasobów. Mogą one być za równo skomplikowane, na przykład mające na celu wprowadzanie dynamicznych zmian w systemie w czasie rzeczywistym, lub proste, jak zwyczajnie, takie jak rozłożenie całej infrastruktury po równo, w każdym urządzeniu.
Zarządzanie zasobami może być także dystrybuowane, w takiej sytuacji każde z urządzeń może się zarządzać samo, bez wiedzy reszty infrastruktury, lub centrlizowane, gdzie wszystkim zarządza jeden system.

\subsection{Warstwa aplikacji}
Jest to warstwa samych aplikacji przeznaczonych do infrsatruktury fog, a przez to do modelu rozproszonego przepływu danych (DDF)~\cite{DDF}. W modelu przepływu danych logikę aplikacji można rozrysować jako ukierunkowany graf, w którym każdy wierzchołek może przyjmować input, output, lub być niezależnym modułem przetwarzającym.
W iFogSim aplikację opisuje klasa Application, w której moduły AppModule mają przypisane wymagania MIPS, RAM i przepustowości. Krawędzie grafu AppEdge łączą sensory, moduły i aktuatory, określając kierunek przepływu danych w górę lub w dół hierarchii, oraz rozmiar przesyłanych krotek (Tuple). Zamknięte ścieżki przetwarzania definiuje AppLoop, co pozwala mierzyć opóźnienie całego cyklu w eksperymentach rozmieszczania modułów.
Taki model dataflow jest wspólną podstawą wszystkich scenariuszy symulacyjnych opisanych dalej w raporcie.

\section{Przegląd literatury}
W literaturze dotyczącej iFogSim można wyróżnić trzy podstawowe publikacje, do których odnosimy się w dalszej części pracy.
Pierwsza z nich~\cite{iFogSimSPE} przedstawia oryginalny toolkit iFogSim jako rozszerzenie CloudSim do modelowania zarządzania zasobami w IoT, Edge i Fog. Autorzy opisują hierarchię urządzeń, model aplikacji DDF oraz sposób rozmieszczania modułów, co stanowi fundament naszych eksperymentów porównujących strategię Cloud i Fog. Należy jednak pamiętać, że oryginalny artykuł nie uwzględniał mobilności urządzeń, a model energii pozostaje uproszczony, gdyż wyniki symulacji zależą od ręcznego wymiarowania parametrów MIPS.
Kolejna praca~\cite{iFogSim2JSS} rozwija symulator o mobilność, klastrowanie mikroserwisów i migrację, co jest istotne zwłaszcza dla scenariusza CrowdSensing. Z analizy tej publikacji wynika, że samo rozszerzenie symulatora o mobilność nie oznacza jeszcze automatycznej przewagi Fog nad Cloud. Nasze wyniki CrowdSensing pokazują to wyraźnie: przy małej liczbie użytkowników dominuje narzut migracji, w średniej skali widać korzyść z lokalnego przetwarzania, a przy dużym obciążeniu brzeg znów traci przewagę. Wadą tego rozszerzenia jest przy tym rosnąca złożoność konfiguracji, gdyż koszt migracji modułów może zdominować korzyść Fog przy małej skali obciążenia.
Rozdział tutorialowy~\cite{iFogSimTutBook} w opracowaniu Buyya i Srirama porządkuje sposób budowy topologii, definiowania aplikacji i zbierania metryk, takich jak latencja, energia czy wykorzystanie sieci. W naszym projekcie zastosowaliśmy podobne podejście: jawne definiowanie pętli aplikacyjnych, zapis wyników do plików CSV oraz porównanie placementu Cloud-only z placementem Fog opartym na ModulePlacementEdgewards. Tutorial opisuje API symulatora, lecz nie dostarcza walidacji na danych produkcyjnych ani gotowych benchmarków dla wszystkich domen IoT, dlatego w projekcie sami zdefiniowaliśmy scenariusze eksperymentalne.
...
Kod źródłowy iFogSim podzielony jest na pakiety odpowiadające poszczególnym warstwom symulatora. Tabela~\ref{tab:packages} zestawia najważniejsze z nich wraz z rolami i klasami używanymi w naszych eksperymentach.
iFogSim dziedziczy po CloudSim model zdarzeń dyskretnych --- DES, w którym cała symulacja to sekwencja zdarzeń kolejkowanych przez silnik \texttt{CloudSim}. Encje symulacji --- urządzenia Fog, sensory i aktuatory --- rejestruje \texttt{FogBroker}, a przetwarzanie krotek oraz rozliczenie zużycia zasobów odbywa się asynchronicznie w pętli zdarzeń.
...
Dla CrowdSensing uruchomiono 28 różnych liczb użytkowników (od 1 do 100) w obu strategiach, co dało 56 wierszy w pliku CSV. Dla DCNS przetestowano 42 konfiguracje obszarów i kamer, również w wersji Cloud i Fog, czyli 84 wiersze.
